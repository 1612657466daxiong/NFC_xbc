 ...
 * 类名：ImageLoader
 * 图片加载缓存工具类
 * Created by xiongqw on 2016/12/11.
 */
...
//        mBean.url=url;
     ...
     * @return
     */
    public ImageLoader saveFileName(String name) {
        mBean.saveFileName=name;
        return this;
    }
    ...
    public Bitmap loadImage(final Context context) {
        ...
        Request request;
        try {
            request = new Request.Builder().url(mBean.url).tag(mTag).build();
...
//                            byte[] bitmapByte = ImageUtils.getBitmapByte(bitmap);
//                            String s = Base64.encodeToString(bitmapByte, Base64.DEFAULT);
//
//                            if (pageDao.isUrllimit(mBean.url)){
//                                pageDao.updateUrlContent(new UrlContent(mBean.url,s));
//                            }else {
//                                pageDao.saveUrlcontent(new UrlContent(mBean.url,s));
//                            }
//                        }
    ...
    /**
     * 则设置图片下载后 主线程默认的图片显示代码->mOnImageLoadListener
     ...
     * @param context
     */
    public ImageLoader showImage(Context context) {
